...
/**
 * Note that this integration tests rely on other methods working correctly, so
 * these tests aren't independent unit tests for all methods, they are intended
 * to test the completeness of the functionality of each API version
 * implementation.
 *
...
public class CloudFoundryClientTest {
    ...
    private static final String CCNG_API_URL = System.getProperty(
            "ccng.target", "http://api.run.pivotal.io");

    private static final String CCNG_API_PROXY_HOST = System.getProperty(
            "http.proxyHost", null);

    private static final int CCNG_API_PROXY_PORT = Integer.getInteger(
            "http.proxyPort", 80);
    ...
    private static final String TEST_DOMAIN = System.getProperty(
            "vcap.test.domain", defaultNamespace(CCNG_USER_EMAIL) + ".com");

    private static final String MYSQL_SERVICE_LABEL = System.getProperty(
            "vcap.mysql.label", "cleardb");
    private static final String MYSQL_SERVICE_PLAN = System.getProperty(
            "vcap.mysql.plan", "spark");

    private static final String DEFAULT_STACK_NAME = "lucid64";

    private static final boolean SILENT_TEST_TIMINGS = Boolean
            .getBoolean("silent.testTimings");
    ...
    public static void beforeClass() throws Exception {
        System.out.println("Running tests on " + CCNG_API_URL
                + " on behalf of " + CCNG_USER_EMAIL);
        System.out.println("Using space " + CCNG_USER_SPACE
                + " of organization " + CCNG_USER_ORG);
        ...
        if (CCNG_API_PROXY_HOST != null) {
            httpProxyConfiguration = new HttpProxyConfiguration(
                    CCNG_API_PROXY_HOST, CCNG_API_PROXY_PORT);
        }
        if (!SKIP_INJVM_PROXY) {
            new SocketDestHelper().setForbiddenOnCurrentThread();
            startInJvmProxy();
            httpProxyConfiguration = new HttpProxyConfiguration("127.0.0.1",
                    inJvmProxyPort);
        }
    }
    ...
    public void setUp() throws Exception {
        ...
        if (nbInJvmProxyRcvReqs != null) {
            nbInJvmProxyRcvReqs.set(0); // reset calls made in setup to leave a
                                        // clean state for tests to assert
        }
    }

    @After
    public void tearDown() throws Exception {
        // Clean after ourselves so that there are no leftover apps, services,
        // domains, and routes
    ...
    /**
     * Self tests that the assert mechanisms with jetty and byteman are properly
     * working. If debugging is needed consider enabling one or more of the
     * following system properties -Dorg.jboss.byteman.verbose=true
     * -Dorg.jboss.byteman.debug=true -Dorg.jboss.byteman.rule.debug=true
     * -Dorg.eclipse.jetty.util.log.class=org.eclipse.jetty.util.log.StdErrLog
     * -Dorg.eclipse.jetty.LEVEL=INFO -Dorg.eclipse.jetty.server.LEVEL=INFO
     * -Dorg.eclipse.jetty.server.handler.ConnectHandler=DEBUG Documentation on
     * byteman at
     * http://downloads.jboss.org/byteman/2.1.3/ProgrammersGuideSinglePage
     * .2.1.3.1.html
     */
    @Test
    public void checkByteManrulesAndInJvmProxyAssertMechanisms() {
        ...
        // when called directly without a proxy, and we configure byteman to
        // detect them
        // then we expect an exception to be thrown
        assertNetworkCallFails(restTemplate,
                new HttpComponentsClientHttpRequestFactory());
        // Repeat that with different request factory used in the code as this
        // exercise different byteman rules
        assertNetworkCallFails(restTemplate,
                new SimpleClientHttpRequestFactory());
        // And with the actual one used by RestUtil
        assertNetworkCallFails(restTemplate,
                new RestUtil().createRequestFactory(null, false));

        // when called with a proxy
        requestFactory = new HttpComponentsClientHttpRequestFactory();// avoid
                                                                      // reusing
                                                                      // keep
                                                                      // alive
                                                                      // connections
        ...
        commonsFactory.getHttpClient().getParams()
                .setParameter(ConnRoutePNames.DEFAULT_PROXY, proxy);
        restTemplate.setRequestFactory(requestFactory);
        restTemplate
                .execute(CCNG_API_URL + "/info", HttpMethod.GET, null, null);

        // then executes fines, and the jetty proxy indeed received one request
        assertEquals(
                "expected network calls to make it through the inJvmProxy.", 1,
        ...
        assertFalse(
                "expected some installed rules, got:"
                        + SocketDestHelper.getInstalledRules(),
                SocketDestHelper.getInstalledRules().isEmpty());
    }

    private void assertNetworkCallFails(RestTemplate restTemplate,
            ClientHttpRequestFactory requestFactory) {
        restTemplate.setRequestFactory(requestFactory);
        try {
            HttpStatus status = restTemplate.execute(CCNG_API_URL + "/info",
                    HttpMethod.GET, null, new ResponseExtractor<HttpStatus>() {
                        public HttpStatus extractData(
                                ClientHttpResponse response) throws IOException {
                            return response.getStatusCode();
                        }
                    });
            Assert.fail("Expected byteman rules to detect direct socket connections, status is:"
                    + status);
        } catch (Exception e) {
        ...
        assertEquals(
                "Not expecting Jetty to receive requests since we asked direct connections",
                0, nbInJvmProxyRcvReqs.get());
    }
        ...
        while (!PortAvailability.available(current)) {
            ...
            if (current - initial > 100) {
                throw new RuntimeException(
                        "did not find an available port from " + initial
                                + " up to:" + current);
            }
        }
        return current;
    ...
    /**
     * To test that the CF client is able to go through a proxy, we point the CC
     * client to a broken url that can only be resolved by going through an
     * inJVM proxy which rewrites the URI. This method starts this inJvm proxy.
     * 
     * @throws Exception
     */
    private static void startInJvmProxy() throws Exception {
        inJvmProxyPort = getNextAvailablePort(8080);
        inJvmProxyServer = new Server(new InetSocketAddress("127.0.0.1",
                inJvmProxyPort)); // forcing use of loopback that will be used
                                  // both for Httpclient proxy and
                                  // SocketDestHelper
        ...
        ChainedProxyServlet chainedProxyServlet = new ChainedProxyServlet(
                httpProxyConfiguration, nbInJvmProxyRcvReqs);
    ...
    public void createApplication() {
        ...
        connectedClient.createApplication(appName, staging, DEFAULT_MEMORY,
                uris, null);
    ...
    public void getApplications() {
        final String serviceName = "test_database";
        String appName = createSpringTravelApp("2",
                Collections.singletonList(serviceName));
    ...
    public void paginationWorksForUris() throws IOException {
        ...
        assertEquals(Collections.singletonList(computeAppUrl(appName)),
                originalUris);
    ...
    public void setEnvironmentThroughList() throws IOException {
        ...
        connectedClient.updateApplicationEnv(appName,
                asList("foo=bar", "bar=baz"));
        app = connectedClient.getApplication(app.getName());
        assertEquals(new HashSet<String>(asList("foo=bar", "bar=baz")),
                new HashSet<String>(app.getEnv()));

        connectedClient.updateApplicationEnv(appName,
                asList("foo=baz", "baz=bong"));
        app = connectedClient.getApplication(app.getName());
        assertEquals(new HashSet<String>(asList("foo=baz", "baz=bong")),
                new HashSet<String>(app.getEnv()));
    ...
    public void updateApplicationUris() throws IOException {
        ...
        List<String> originalUris = app.getUris();
        assertEquals(
                Collections.singletonList(computeAppUrlNoProtocol(appName)),
                originalUris);
    ...
    public void updateStandaloneApplicationCommand() throws IOException {
        ...
        Staging newStaging = new Staging("ruby simple.rb test",
        ...
        assertEquals("https://github.com/cloudfoundry/heroku-buildpack-ruby",
                app.getStaging().getBuildpackUrl());
    }
    ...
    public void getApplicationsMatchGetApplication() {
        ...
        assertEquals(app.getMeta().getCreated(), apps.get(0).getMeta()
                .getCreated());
        assertEquals(app.getMeta().getUpdated(), apps.get(0).getMeta()
                .getUpdated());
    ...
    public void getApplicationInstances() throws Exception {
        ...
        boolean passSingleInstance = getInstanceInfosWithTimeout(appName, 1,
                true);
        assertTrue("Couldn't get the right application state in 50 tries",
                passSingleInstance);

        boolean passSingleMultipleInstances = getInstanceInfosWithTimeout(
                appName, 3, true);
        assertTrue("Couldn't get the right application state in 50 tries",
                passSingleMultipleInstances);

        connectedClient.stopApplication(appName);
        InstancesInfo instInfo = connectedClient
    ...
    public void getCrashes() throws IOException, InterruptedException {
        ...
        connectedClient.updateApplicationEnv(appName,
                Collections.singletonMap("crash", "true"));
        connectedClient.startApplication(appName);
    ...
    public void accessRandomApplicationUrl() throws Exception {
        ...
        RestTemplate rest = restUtil.createRestTemplate(httpProxyConfiguration,
                false);
        String results = rest.getForObject("http://" + app.getUris().get(0),
                String.class);
    ...
    public void getApplicationStats() throws Exception {
        ...
        for (InstanceStats instanceStats : stats.getRecords()) {
            ...
            assertTimeWithinRange("Usage time should be very recent", usage
                    .getTime().getTime(), FIVE_MINUTES);
        }
    }

    private void waitForStatsAvailable(String appName, int instanceCount)
            throws InterruptedException {
        // TODO: Make this pattern reusable
        ApplicationStats stats = connectedClient.getApplicationStats(appName);
        for (int retries = 0; retries < 10
                && stats.getRecords().size() < instanceCount; retries++) {
            Thread.sleep(1000);
    ...
    public void uploadAppWithNonUnsubscribingCallback() throws IOException {
        ...
        assertTrue(callback.progressCount > 1); // must have taken at least 10
                                                // seconds
    }
    ...
    public void getServiceOfferings() {
        List<CloudServiceOffering> offerings = connectedClient
                .getServiceOfferings();
    ...
    public void getCreateDeleteService() throws MalformedURLException {
        ...
        assertTimeWithinRange("Creation time should be very recent", service
                .getMeta().getCreated().getTime(), FIVE_MINUTES);
    ...
    private void assertServiceMatching(CloudService expectedService,
    ...
    private void assertServicesEqual(CloudService expectedService,
    ...
    public void createApplicationWithService() throws IOException {
        ...
        String appName = createSpringTravelApp("application-with-services",
                Collections.singletonList(serviceName));
    ...
    public void deleteServiceThatIsBoundToApp() throws MalformedURLException {
        ...
        String appName = createSpringTravelApp("del-svc",
                Collections.singletonList(serviceName));
    ...
    public void updateApplicationService() throws IOException {
        ...
        connectedClient.updateApplicationServices(appName,
                Collections.singletonList(serviceName));
        CloudApplication app = connectedClient.getApplication(appName);
        assertNotNull(app.getServices());
    ...
    public void appsWithRoutesAreCounted() throws IOException {
        ...
        assertEquals(1, getRouteWithHost("my_route3", routes)
                .getAppsUsingRoute());
        assertTrue(getRouteWithHost("my_route3", routes).inUse());

        List<CloudRoute> defaultDomainRoutes = connectedClient
                .getRoutes(defaultDomainName);
        assertNotNull(getRouteWithHost(appName, defaultDomainRoutes));
        assertEquals(1, getRouteWithHost(appName, defaultDomainRoutes)
                .getAppsUsingRoute());
    ...
    public void infoAvailableWithoutLoggingIn() throws Exception {
        CloudFoundryClient infoClient = new CloudFoundryClient(new URL(
                CCNG_API_URL), httpProxyConfiguration);
    ...
    public void infoForUserAvailable() throws Exception {
        ...
        assertTrue(info.getLimits().getMaxApps() > 0
                && info.getLimits().getMaxApps() < 1000);
        assertTrue(info.getLimits().getMaxServices() > 0
                && info.getLimits().getMaxServices() < 1000);
        assertTrue(info.getLimits().getMaxTotalMemory() > 0
                && info.getLimits().getMaxTotalMemory() < 100000);
        assertTrue(info.getLimits().getMaxUrisPerApp() > 0
                && info.getLimits().getMaxUrisPerApp() < 100);
    }
    ...
    public void updatePassword() throws MalformedURLException {
        ...
        CloudFoundryClient clientWithChangedPassword = new CloudFoundryClient(
                new CloudCredentials(CCNG_USER_EMAIL, newPassword), new URL(
                        CCNG_API_URL), httpProxyConfiguration);
    ...
    public void dealingWithExpiredToken() throws Exception {
        // The current token expiration time is 10 minutes. If we can still make
        // authenticated calls past that,
        // then the transparent token refresh scheme working as expected.
        for (int i = 0; i < 30; i++) {
            System.out.println("Elapsed time since the last login (at least) "
                    + i / 2 + " minutes");
            getServiceOfferings();
    ...
    private boolean getInstanceInfosWithTimeout(String appName, int count,
                ...
                for (InstanceInfo info : infos) {
                    if (shouldBeRunning) {
                        if (InstanceState.RUNNING.equals(info.getState())
                                || InstanceState.STARTING.equals(info
                                        .getState())) {
                            passCount++;
                        }
                    } else {
                        if (InstanceState.CRASHED.equals(info.getState())
                                || InstanceState.FLAPPING.equals(info
                                        .getState())) {
                            passCount++;
                        }
                    }
    ...
    private void doGetFile(CloudFoundryClient client, String appName)
            throws Exception {
        String appDir = "app";
        String fileName = appDir + "/WEB-INF/web.xml";
        String emptyPropertiesFileName = appDir
                + "/WEB-INF/classes/empty.properties";
        ...
        // Test downloading range of file with start position and end position
        // exceeding the length
        int positionPastEndPosition = fileContent.length() + 999;
        String fileContent4 = client.getFile(appName, 0, fileName, start,
                positionPastEndPosition);
        ...
        assertEquals(fileContent.substring(fileContent.length() - length),
                fileContent5);

        // Test downloading one byte of file with start and end position
        String fileContent6 = client.getFile(appName, 0, fileName, start,
                start + 1);
        ...
        try {
            ...
            assertTrue(e.getStatusCode().equals(
                    HttpStatus.REQUESTED_RANGE_NOT_SATISFIABLE));
        }

        // Test downloading empty file
        String fileContent7 = client.getFile(appName, 0,
                emptyPropertiesFileName);
        assertNull(fileContent7);

        // Test downloading with invalid parameters - should all throw
        // exceptions
        try {
    ...
    private CloudApplication createAndUploadSimpleTestApp(String name)
            throws IOException {
    ...
    private CloudApplication uploadSpringTravelApp(String appName)
            throws IOException {
        File file = SampleProjects.springTravel();
        connectedClient.uploadApplication(appName, file.getCanonicalPath());
    ...
    private CloudApplication createAndUploadExplodedSpringTestApp(String appName)
        ...
        assertTrue(
                "Expected exploded test app at "
                        + explodedDir.getCanonicalPath(), explodedDir.exists());
        createTestApp(appName, null, new Staging());
        connectedClient.uploadApplication(appName,
                explodedDir.getCanonicalPath());
        return connectedClient.getApplication(appName);
    }

    private CloudApplication createAndUploadAndStartSimpleSpringApp(
    ...
    private CloudApplication createAndUploadSimpleSpringApp(String appName)
            throws IOException {
        createSpringApplication(appName);
    ...
    private CloudApplication createAndUploadExplodedTestApp(String appName,
            File explodedDir, Staging staging) throws IOException {
        assertTrue(
                "Expected exploded test app at "
                        + explodedDir.getCanonicalPath(), explodedDir.exists());
    ...
    private void createStandaloneRubyTestApp(String appName, List<String> uris,
        ...
        connectedClient
                .createApplication(appName, staging, 128, uris, services);
        connectedClient.uploadApplication(appName, file.getCanonicalPath());
    }
    ...
    private void createSpringApplication(String appName, String stack,
            Integer healthCheckTimeout) {
        createTestApp(appName, null, new Staging(null, null, stack,
                healthCheckTimeout));
    }

    private void createTestApp(String appName, List<String> serviceNames,
            Staging staging) {
        List<String> uris = new ArrayList<String>();
        uris.add(computeAppUrl(appName));
    ...
    private CloudService createMySqlServiceWithVersionAndProvider(
            String serviceName) {
    ...
    private CloudServiceOffering getCloudServiceOffering(String label) {
        List<CloudServiceOffering> serviceOfferings = connectedClient
                .getServiceOfferings();
        ...
        throw new IllegalStateException(
                "No CloudServiceOffering found with label " + label + ".");
    }

    private InstancesInfo getInstancesWithTimeout(CloudFoundryClient client,
            String appName) {
        ...
        while (true) {
            ...
            final InstancesInfo applicationInstances = client
                    .getApplicationInstances(appName);
    ...
    private void clearTestDomainAndRoutes() {
        CloudDomain domain = getDomainNamed(TEST_DOMAIN,
                connectedClient.getDomains());
        if (domain != null) {
            List<CloudRoute> routes = connectedClient.getRoutes(domain
                    .getName());
    ...
    private CloudDomain getDomainNamed(String domainName,
    ...
    private void assertTimeWithinRange(String message, long actual,
            int timeTolerance) {
        // Allow more time deviations due to local clock being out of sync with
        // cloud
        assertTrue(message,
                Math.abs(System.currentTimeMillis() - actual) < timeTolerance);
    }

    private static abstract class NoOpUploadStatusCallback
            implements
    ...
    private static class NonUnsubscribingUploadStatusCallback
            extends
                NoOpUploadStatusCallback {
    ...
    private static class UnsubscribingUploadStatusCallback
            extends
                NoOpUploadStatusCallback {
        public int progressCount = 0;
